...
export function Admin({ apiBase, content, onContent, navigate, sourcePages }) {
  ...
  const videoCount = draft.videos.length + Object.values(draft.pageOverrides || {}).reduce((total, page) => total + (page.videos?.filter((video) => video.url).length || 0), 0);
  ...
  const selectedMedia = override.media ?? sourcePage?.media?.map((item) => item.local) ?? [];
  const selectedVideos = override.videos ?? sourcePage?.videos ?? [];
    ...
    return (
      <div className="admin-gate">
        ...
        <form className="admin-login-card" onSubmit={login}>
          <img className="admin-login-logo" src={assetUrl(draft.brand.logoDark || draft.brand.logo)} alt={draft.brand.name} />
  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main">
        ...
        {activeTab === "overview" && <section className="admin-panel"><div className="admin-welcome"><span>PRODUCT-FIRST CONTENT</span><h2>Keep PURIUM technology in focus.</h2><p>Use the editors to update Nova’s positioning, contact details, every page’s copy, imagery and video library. Changes are stored by the Nova server and become visible immediately.</p></div><div className="admin-stat-grid"><article><strong>{routes.length}</strong><span>Editable public pages</span></article><article><strong>{videoCount}</strong><span>Published videos</span></article><article><strong>{inquiries.length}</strong><span>Loaded inquiries</span></article></div></section>}

        {activeTab === "identity" && (
          <section className="admin-panel admin-form-grid">
            <div>
              <h2>Brand identity</h2>
              <Field label="Organization name"><input value={draft.brand.name} onChange={(e) => setPath(["brand", "name"], e.target.value)} /></Field>
              <Field label="Partner statement"><input value={draft.brand.partnerLabel} onChange={(e) => setPath(["brand", "partnerLabel"], e.target.value)} /></Field>
              <Field label="Compact logo URL"><input value={draft.brand.logo} onChange={(e) => setPath(["brand", "logo"], e.target.value)} /></Field>
              <Field label="Upload a compact logo" hint="PNG, JPG or WebP"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) { setPath(["brand", "logo"], url); setNotice("Logo uploaded. Save changes to publish it."); } } catch (error) { setNotice(error.message); } }} /></Field>
              <Field label="Dark-surface logo URL"><input value={draft.brand.logoDark || ""} onChange={(e) => setPath(["brand", "logoDark"], e.target.value)} /></Field>
              <Field label="Light-surface logo URL"><input value={draft.brand.logoLight || ""} onChange={(e) => setPath(["brand", "logoLight"], e.target.value)} /></Field>
              <Field label="Parent company link label"><input value={draft.brand.parentCompanyLabel} onChange={(e) => setPath(["brand", "parentCompanyLabel"], e.target.value)} /></Field>
              <Field label="Parent company website"><input type="url" value={draft.brand.parentCompanyUrl} onChange={(e) => setPath(["brand", "parentCompanyUrl"], e.target.value)} /></Field>
            </div>
            <div>
              <h2>Homepage content</h2>
              <Field label="Eyebrow"><input value={draft.home.eyebrow} onChange={(e) => setPath(["home", "eyebrow"], e.target.value)} /></Field>
              <Field label="Headline — one line per row"><textarea rows="4" value={draft.home.headline.join("\n")} onChange={(e) => setPath(["home", "headline"], splitLines(e.target.value))} /></Field>
              <Field label="Description"><textarea rows="5" value={draft.home.description} onChange={(e) => setPath(["home", "description"], e.target.value)} /></Field>
              <Field label="Hero image URL"><input value={draft.home.heroImage} onChange={(e) => setPath(["home", "heroImage"], e.target.value)} /></Field>
              <Field label="Upload a hero image" hint="PNG, JPG or WebP"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) { setPath(["home", "heroImage"], url); setNotice("Hero image uploaded. Save changes to publish it."); } } catch (error) { setNotice(error.message); } }} /></Field>
              <Field label="Technology heading"><input value={draft.home.technologyHeading} onChange={(e) => setPath(["home", "technologyHeading"], e.target.value)} /></Field>
              <Field label="Technology description"><textarea rows="4" value={draft.home.technologyCopy} onChange={(e) => setPath(["home", "technologyCopy"], e.target.value)} /></Field>
              <Field label="Products heading"><input value={draft.home.productsHeading} onChange={(e) => setPath(["home", "productsHeading"], e.target.value)} /></Field>
              <Field label="Products description"><textarea rows="4" value={draft.home.productsCopy} onChange={(e) => setPath(["home", "productsCopy"], e.target.value)} /></Field>
            </div>
          </section>
        )}
        ...
        {activeTab === "pages" && <section className="admin-panel"><div className="page-editor-toolbar"><Field label="Choose a page"><select value={selectedRoute} onChange={(e) => setSelectedRoute(e.target.value)}>{routes.map((page) => <option key={page.route} value={page.route}>{page.title.replace(/\n/g, " ")} — {page.route}</option>)}</select></Field><span>{selectedRoute}</span></div><div className="admin-form-grid"><div><Field label="Page title"><input value={override.title ?? sourcePage?.title ?? ""} onChange={(e) => setOverride("title", e.target.value)} /></Field><Field label="Eyebrow"><input value={override.eyebrow ?? sourcePage?.eyebrow ?? ""} onChange={(e) => setOverride("eyebrow", e.target.value)} /></Field><Field label="Section headings — one per line"><textarea rows="7" value={(override.headings ?? sourcePage?.headings ?? []).join("\n")} onChange={(e) => setOverride("headings", splitLines(e.target.value))} /></Field><Field label="Structured items — blank line between items"><textarea rows="12" value={(override.lists ?? sourcePage?.lists ?? []).join("\n\n")} onChange={(e) => setOverride("lists", splitParagraphs(e.target.value))} /></Field></div><div><Field label="Body paragraphs — blank line between paragraphs"><textarea rows="16" value={(override.paragraphs ?? sourcePage?.paragraphs ?? []).join("\n\n")} onChange={(e) => setOverride("paragraphs", splitParagraphs(e.target.value))} /></Field><Field label="Image URLs — one per line"><textarea rows="6" value={selectedMedia.join("\n")} onChange={(e) => setOverride("media", splitLines(e.target.value))} /></Field><Field label="Upload and add a page image" hint="The uploaded image is appended to this page"><input type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) { setOverride("media", [...selectedMedia, url]); setNotice("Page image uploaded. Save changes to publish it."); } } catch (error) { setNotice(error.message); } }} /></Field><div className="page-video-editor"><h3>Videos on this page</h3><p>Use an uploaded MP4/WebM URL or a YouTube/Vimeo link.</p>{selectedVideos.map((video, index) => <div className="video-editor" key={`${video.url}-${index}`}><input placeholder="Video title" value={video.title || ""} onChange={(e) => { const next = clone(selectedVideos); next[index].title = e.target.value; setOverride("videos", next); }} /><input placeholder="https://…" value={video.url || ""} onChange={(e) => { const next = clone(selectedVideos); next[index].url = e.target.value; setOverride("videos", next); }} /><button type="button" onClick={() => setOverride("videos", selectedVideos.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></div>)}<button type="button" className="button secondary" onClick={() => setOverride("videos", [...selectedVideos, { title: "PURIUM technology", url: "" }])}><FileVideo size={17} /> Add page video</button></div></div></div></section>}

        {activeTab === "media" && <section className="admin-panel"><div className="admin-form-grid"><div><h2>Upload media</h2><p>Upload a photo, logo or video, then add the returned URL to a page or brand field.</p><label className="upload-drop"><Upload size={28} /><strong>Choose an image or video</strong><span>JPG, PNG, WebP, GIF, MP4 or WebM</span><input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) setNotice(`Uploaded: ${url}`); } catch (error) { setNotice(error.message); } }} /></label></div><div><h2>Homepage video showcase</h2><p>Add uploaded video URLs or supported YouTube/Vimeo links. Videos tied to a specific page are managed in Page editor.</p>{draft.videos.map((video, index) => <div className="video-editor" key={`${video.url}-${index}`}><input placeholder="Video title" value={video.title} onChange={(e) => { const next = clone(draft.videos); next[index].title = e.target.value; setPath(["videos"], next); }} /><input placeholder="https://…" value={video.url} onChange={(e) => { const next = clone(draft.videos); next[index].url = e.target.value; setPath(["videos"], next); }} /><button type="button" onClick={() => setPath(["videos"], draft.videos.filter((_, itemIndex) => itemIndex !== index))}>Remove</button></div>)}<button type="button" className="button secondary" onClick={() => setPath(["videos"], [...draft.videos, { title: "PURIUM technology", url: "" }])}><FileVideo size={17} /> Add homepage video</button></div></div></section>}
