  ...
  return (
    <div className="admin-shell">
      ...
      <main className="admin-main">
        ...
        {activeTab === "identity" && <section className="admin-panel admin-form-grid"><div><h2>Brand identity</h2><Field label="Organization name"><input value={draft.brand.name} onChange={(e) => setPath(["brand", "name"], e.target.value)} /></Field><Field label="Partner statement"><input value={draft.brand.partnerLabel} onChange={(e) => setPath(["brand", "partnerLabel"], e.target.value)} /></Field><Field label="Logo URL"><input value={draft.brand.logo} onChange={(e) => setPath(["brand", "logo"], e.target.value)} /></Field><Field label="Upload a logo" hint="PNG, JPG or WebP"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) { setPath(["brand", "logo"], url); setNotice("Logo uploaded. Save changes to publish it."); } } catch (error) { setNotice(error.message); } }} /></Field><Field label="Parent company link label"><input value={draft.brand.parentCompanyLabel} onChange={(e) => setPath(["brand", "parentCompanyLabel"], e.target.value)} /></Field><Field label="Parent company website"><input type="url" value={draft.brand.parentCompanyUrl} onChange={(e) => setPath(["brand", "parentCompanyUrl"], e.target.value)} /></Field></div><div><h2>Homepage content</h2><Field label="Eyebrow"><input value={draft.home.eyebrow} onChange={(e) => setPath(["home", "eyebrow"], e.target.value)} /></Field><Field label="Headline — one line per row"><textarea rows="4" value={draft.home.headline.join("\n")} onChange={(e) => setPath(["home", "headline"], splitLines(e.target.value))} /></Field><Field label="Description"><textarea rows="5" value={draft.home.description} onChange={(e) => setPath(["home", "description"], e.target.value)} /></Field><Field label="Hero image URL"><input value={draft.home.heroImage} onChange={(e) => setPath(["home", "heroImage"], e.target.value)} /></Field><Field label="Upload a hero image" hint="PNG, JPG or WebP"><input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (e) => { try { const url = await uploadMedia(e.target.files[0]); if (url) { setPath(["home", "heroImage"], url); setNotice("Hero image uploaded. Save changes to publish it."); } } catch (error) { setNotice(error.message); } }} /></Field><Field label="Technology heading"><input value={draft.home.technologyHeading} onChange={(e) => setPath(["home", "technologyHeading"], e.target.value)} /></Field><Field label="Technology description"><textarea rows="4" value={draft.home.technologyCopy} onChange={(e) => setPath(["home", "technologyCopy"], e.target.value)} /></Field><Field label="Products heading"><input value={draft.home.productsHeading} onChange={(e) => setPath(["home", "productsHeading"], e.target.value)} /></Field><Field label="Products description"><textarea rows="4" value={draft.home.productsCopy} onChange={(e) => setPath(["home", "productsCopy"], e.target.value)} /></Field></div></section>}
